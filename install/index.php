...
switch($step)
{
    ...
    case 4:
        ...
        $db_host = trim(request('db_host', 'localhost', 'post'));
        $db_port = trim(request('db_port', '3306', 'post'));
        $db_user = trim(request('db_user', 'root', 'post'));
        $db_pass = trim(request('db_pass', '', 'post'));
        $db_name = trim(request('db_name', null, 'post'));
        $db_table_pre = trim(request('db_table_pre', '', 'post'));
        $admin_username = trim(request('admin_username', null, 'post'));
        $admin_password = trim(request('admin_password', null, 'post'));
        $admin_repassword = trim(request('admin_repassword', null, 'post'));
        $admin_email = trim(request('admin_email', null, 'post'));
        ...
        $streams = str_replace('#http_host#', get_http_host(), $streams);
        ...
        $r = write_config($configs, 'config.php') !== FALSE ? 0 : 1;
        $install_errors += $r;
        ...
        if(request('sample_data', 0, 'post') == 1)
        {
        ...
        if($install_errors == 0)
        {
            ...
            call_script('finished', 'success', '安装完成');
        }
        else
        {
