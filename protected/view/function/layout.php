...
{
    $layout = new layout();
    if($html = $layout->check_static_file('topper.html')) return $html;
    
    $vcache = new vcache();
    $nav = $vcache->nav_model('get_site_nav');
    $assigns['nav'] = $nav['top'];
    $assigns = $assigns + $params;
    return $layout->tpl_render('topper.html', $assigns, TRUE);
}

function layout_header($params = array())
{
    $layout = new layout();
    if($html = $layout->check_static_file('header.html')) return $html;
    ...
    $assigns = $assigns + $params;
    return $layout->tpl_render('header.html', $assigns, TRUE);
}

function layout_helper()
{
    $layout = new layout();
    if($html = $layout->check_static_file('helper.html')) return $html;
    
    $vcache = new vcache();
    $assigns['helper_list'] = $vcache->help_model('cated_help_list');
    return $layout->tpl_render('helper.html', $assigns, TRUE);
}

function layout_footer()
{
    $layout = new layout();
    if($html = $layout->check_static_file('footer.html')) return $html;
    ...
    return $layout->tpl_render('footer.html', $assigns, TRUE);
}

function layout_catebar()
{
    $layout = new layout();
    if($html = $layout->check_static_file('catebar.html')) return $html;
    
    $vcache = new vcache();
    return $layout->tpl_render('catebar.html', array('catebar' => $vcache->goods_cate_model('goods_cate_bar')), TRUE);    
}
...
function layout_login()
{
    ...
    return $layout->tpl_render('poplogin.html', $assigns, FALSE);
}
...
function layout_usermenu()
{
    ...
    (
        ...
        array('name' => '我的订单', 'c' => 'user', 'a' => 'order'),
    ...
    return $layout->tpl_render('usermenu.html', $assigns, FALSE);
}
