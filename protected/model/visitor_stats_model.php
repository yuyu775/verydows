...
class visitor_stats_model extends Model
{
    ...
    private function check_data($data)
    {
        ...
        {
             $data['sessid'] = md5(session_id());
             setcookie('vds_uvid', $data['sessid'], strtotime('today') + 86400);
        }
        if(!preg_match('/^(?=\d+\.\d+\.\d+\.\d+$)(?:(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\.?){4}$/', $data['ip'])) $data['ip'] = vds_get_ip();
        if(empty($data['referrer'])) $data['referrer'] = self::get_visitor_referer();
        if(!isset($this->platform_map[$data['platform']])) $data['platform'] = self::get_visitor_platform();
        if(!isset($this->platform_map[$data['browser']])) $data['browser'] = self::get_visitor_browser();
        return $data;
    }
    ...
    public function do_stats($data)
    {
        ...
        $exists = array('sessid' => $data['sessid'], 'ip' => $data['ip'], 'dateline' => $today);
        if(!$this->find($exists))
        {
            $data['dateline'] = $today;
            $this->create($data);
        ...
        {
            $this->incr($exists, 'pv', 1);
        }
    }
    
    private static function get_visitor_browser()
    {
    ...
    private static function get_visitor_platform()
    { 
        $agent = $_SERVER['HTTP_USER_AGENT'];
    ...
    private static function get_visitor_referer()
    {
