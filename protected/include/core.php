...
if(!empty($GLOBALS['cfg']['rewrite_enable']) && strpos($_SERVER['REQUEST_URI'], 'index.php?') === FALSE){
...
function url($c = 'main', $a = 'index', $param = array()){
	...
	if(!empty($GLOBALS['cfg']['rewrite_enable']) && $m == ''){
...
class Model{
	...
	public function __construct($table_name = null){
       ...
       else $this->table_name = $GLOBALS['mysql']['MYSQL_DB_TABLE_PRE'] . $this->table_name;
    }
	...
	public function find($conditions = array(), $sort = null, $fields = '*'){
		$res = $this->find_all($conditions, $sort, $fields, 1);
    ...
    public function pager_section()
    {
        $section = empty($this->page) ? '' : " LIMIT {$this->page['offset']},{$this->page['limit']}";
        return $section;
    }
