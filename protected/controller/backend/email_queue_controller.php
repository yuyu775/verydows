...
class email_queue_controller extends general_controller
{
    public function action_index()
    {
        ...
        $this->results = $queue_model->find_all(null, 'id DESC', '*', array(vds_request('page', 1), 15));
        $this->paging = $queue_model->page;
        $this->tpl_display('email/queue_list.html');
    }
    ...
    public function action_delete()
    {
        ...
        if(!empty($id) && is_array($id))
        {
            $affected = 0;
            $queue_model = new email_queue_model();
            foreach($id as $v) $affected += $queue_model->delete(array('id' => $v));
            $failure = count($id) - $affected;
